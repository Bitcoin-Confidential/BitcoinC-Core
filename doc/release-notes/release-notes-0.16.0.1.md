BitcoinC Core version 0.16.0.1 is now available.
...
BitcoinC Core 0.16 introduces generic support for Hardware Signing Devices to provide key-signing capabilities. 
...
In order to facilitate deterministic generation of stealth addresses between BitcoinC Core and Signing Devices the method used to generate these addresses has been changed.
...
BitcoinC Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
bitcoinc data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile BitcoinC Core is now 4.8. No effort will be
...
- bitcoinc-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- bitcoinc-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for bitcoinc-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to bitcoinc-cli to allow RPC password to be read from standard input (jharvell)
...
- #11621 `1f7695b` [build] Add temp_bitcoinc_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` bitcoinc_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.bitcoinc doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for bitcoinc-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in bitcoinc-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop bitcoincd (OmeGak)
...
- #10529 `331352f` Improve bitcoincd systemd service file (Flowdalic)
