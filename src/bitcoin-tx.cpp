...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinc-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinc-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n" +
            "or:     bitcoinc-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n" +
            "\n";
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_BITCOINC_VERSION)
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");

    if (!tx.IsBitcoinCVersion() && IsBitcoinCTxVersion(newVersion))
    {
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (tx.IsBitcoinCVersion() && amount == 0)
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_BITCOINC_VERSION;
        int startArg;
