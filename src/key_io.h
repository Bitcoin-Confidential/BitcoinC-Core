...
public:
    ...
    bool SetString(const std::string& str);
    std::string ToString() const;
    int CompareTo(const CBase58Data_SC& b58) const;

    bool operator==(const CBase58Data_SC& b58) const { return CompareTo(b58) == 0; }
    bool operator<=(const CBase58Data_SC& b58) const { return CompareTo(b58) <= 0; }
    bool operator>=(const CBase58Data_SC& b58) const { return CompareTo(b58) >= 0; }
    bool operator< (const CBase58Data_SC& b58) const { return CompareTo(b58) <  0; }
    bool operator> (const CBase58Data_SC& b58) const { return CompareTo(b58) >  0; }
 ...
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CSmartCashAddress : public CBase58Data_SC {
public:
 ...
 * A base58-encoded secret key
 */
class CSmartCashSecret : public CBase58Data_SC
{
public:
    void SetKey(const CKey& vchSecret);
    CKey GetKey();
    ...
    CSmartCashSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CSmartCashSecret() {}
};
