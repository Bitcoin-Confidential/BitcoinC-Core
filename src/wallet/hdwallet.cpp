...
bool CHDWallet::ProcessStakingSettings(std::string &sError)
{
    ...
    nStakeCombineThreshold = nStakeCombineThresholdDefault * COIN;
    nStakeSplitThreshold = nStakeSplitThresholdDefault * COIN;
    nMaxStakeCombine = 3;
    ...
    if (nStakeCombineThreshold < nStakeCombineThresholdDefault * COIN || nStakeCombineThreshold > nStakeCombineThresholdDefault * 2 * COIN) {
        AppendError(sError, strprintf("\"stakecombinethreshold\" must be >= %d and <= %d.", nStakeCombineThresholdDefault, nStakeCombineThresholdDefault*2));
        nStakeCombineThreshold = nStakeCombineThresholdDefault * COIN;
    }
    ...
    int nChangePosInOut = -1;
    {
        ...
        for (;;) {
            ...
            for (const auto &coin : setCoins) {
                ...
                if (!ProduceSignature(*this, DUMMY_SIGNATURE_CREATOR_BITCOINC, scriptPubKey, sigdata)) {
...
bool CHDWallet::SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime)
{
    ...
    CKey key;
    pblock->nVersion = BITCOINC_BLOCK_VERSION;
    ...
    for( CTxOutBaseRef &out : tx.vpout ){
        ...
        if( out->nVersion == OUTPUT_STANDARD ){
            ...
            if( fIsCoinStake && !fHaveKey &&  !( fHaveKey = HaveKey(keyId2) ) ){
                strError = "ColdStake address must be from your wallet";
            }
