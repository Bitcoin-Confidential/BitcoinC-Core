...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "BitcoinC extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "BitcoinC extended public key");
    } else
    ...
    if (mode == "import") {
        ...
        if (fBip44) {
            ...
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Import failed - Key must begin with a bitcoinc prefix.");
            }
        }
...
static UniValue extkeyaltversion(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "If the provided ext_key has a Bitcoin prefix the output will be encoded with a BitcoinC prefix.\n"
            "If the provided ext_key has a BitcoinC prefix the output will be encoded with a Bitcoin prefix.");
...
static UniValue getnewextaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "Returns a new BitcoinC ext address for receiving payments.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            ...
            "\nResult:\n"
            "\"address\"              (string) The new bitcoinc extended address\n"
            "\nExamples:\n"
...
static UniValue scanchain(const JSONRPCRequest &request)
{
    ...
    if (!IsDeprecatedRPCEnabled("scanchain")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "scanchain is deprecated and will be fully removed in v0.18. "
            "To use scanchain in v0.17, restart bitcoincd with -deprecatedrpc=scanchain.\n"
...
static UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid BitcoinC address."));
    }
...
static UniValue walletsettings(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(strprintf(
            ...
            "}\n"
            "\"stakingstatus\" {\n"
            "  \"enabled\"                   (bool, optional, default=false) Toggle staking enabled on this wallet.\n"
            "}\n"
            "\"stakingoptions\" {\n"
...
static UniValue createrawbctransaction(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetBitcoinCWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "     [\n"
            "       {\n"
            "         \"address\": \"str\"          (string, required) The bitcoinc address\n"
...
//            "         \"subfee\":bool      (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("createrawbctransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"address\\\":0.01}\"")
            + HelpExampleCli("createrawbctransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"data\\\":\\\"00010203\\\"}\"")
            + HelpExampleRpc("createrawbctransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"address\\\":0.01}\"")
            + HelpExampleRpc("createrawbctransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"data\\\":\\\"00010203\\\"}\"")
        );
    ...
    rawTx.nVersion = BITCOINC_TXN_VERSION;
...
static UniValue fundrawtransactionfrom(const JSONRPCRequest& request)
{
    ...
    tx.nVersion = BITCOINC_TXN_VERSION;
